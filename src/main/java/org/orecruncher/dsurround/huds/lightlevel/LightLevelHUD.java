...
@Mod.EventBusSubscriber(modid = DynamicSurroundings.MODID, value = Dist.CLIENT, bus = Mod.EventBusSubscriber.Bus.FORGE)
public final class LightLevelHUD {
    ...
    public enum ColorSet {
        ...
        ColorSet(@Nonnull final Color safe, @Nonnull final Color caution, @Nonnull final Color hazard, @Nonnull final Color noSpawn) {
            this.safe = safe;
    ...
    protected static float heightAdjustment(@Nonnull final BlockState state, @Nonnull final BlockState below, @Nonnull final BlockPos pos) {
        if (state.getMaterial() == Material.AIR) {
            final VoxelShape shape = below.getCollisionShape(GameUtils.getLevel(), pos.below());
            return shape.isEmpty() ? 0 : (float) shape.max(Direction.Axis.Y) - 1;
        }
        
        final VoxelShape shape = below.getCollisionShape(GameUtils.getLevel(), pos);
        if (shape.isEmpty())
            return 0F;
    ...
    protected static void updateLightInfo(@Nonnull final Vec3 position) {
        ...
        final ClientLevel world = GameUtils.getLevel();
            ...
            for (int dZ = 0; dZ < rangeXZ; dZ++) {
                ...
                for (int dY = 0; dY < rangeY; dY++) {
                    ...
                    if (renderLightLevel(state, lastState)) {
                        ...
                        final boolean mobSpawn = lastState.isValidSpawn(GameUtils.getLevel(), mutable, SpawnPlacements.Type.ON_GROUND, EntityType.ZOMBIE);
    ...
    public static void doTick(@Nonnull final TickEvent.PlayerTickEvent event) {
        ...
        updateLightInfo(event.player.getEyePosition());
    }
