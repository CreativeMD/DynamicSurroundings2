...
public final class SoundFXUtils {
    ...
    static {
        ...
        for (int i = 0; i < REVERB_RAYS; i++) {
            ...
            REVERB_RAY_NORMALS[i] = new Vector3d(Math.cos(latitude) * Math.cos(longitude), Math.cos(latitude) * Math.sin(longitude), Math.sin(latitude)).normalize();
    ...
    public void calculate(@Nonnull final WorldContext ctx) {
        ...
        if (ctx.isNotValid() || !this.source.isEnabled() || !SoundUtils.inRange(ctx.playerEyePosition, this.source.getSound()) || this.source.getPosition().equals(Vector3d.ZERO)) {
            this.clearSettings();
            return;
        }
