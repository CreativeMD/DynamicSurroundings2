...
class CommonStateHandler extends HandlerBase {
    ...
    static {
        ...
        scripts = Pattern.compile("\\r?\\n").splitAsStream(script_lines).map(String::trim).filter(s -> !(StringUtils.isNullOrEmpty(s) || s.startsWith("//")))
    ...
    public void process(@Nonnull final PlayerEntity player) {
        ...
        if (currentTick % 20 == 0) {
            ...
            if (world.getDimensionType().isNatural()) {
                ...
                final Optional<TileEntity> bell = world.loadedTileEntityList.stream().filter(te -> te instanceof BellTileEntity)
                        .filter(te -> te.getPos().distanceSq(data.playerEyePosition.x, data.playerEyePosition.y, data.playerEyePosition.z, true) <= VILLAGE_RANGE).findAny();
                
                // If a bell is found, look for a villager within range
                data.isInVillage = bell.isPresent();
                if (data.isInVillage) {
                    final Optional<Entity> entity = Streams.stream(GameUtils.getWorld().getAllEntities()).filter(e -> e instanceof VillagerEntity)
                            .filter(e -> e.getDistanceSq(data.playerEyePosition.x, data.playerEyePosition.y, data.playerEyePosition.z) <= VILLAGE_RANGE).findAny();
