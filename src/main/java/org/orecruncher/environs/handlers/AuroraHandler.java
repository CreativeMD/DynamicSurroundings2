...
public final class AuroraHandler extends HandlerBase {
    ...
    private boolean canAuroraStay() {
        ...
        return isAuroraTimeOfDay() && AuroraUtils.getChunkRenderDistance() >= 6 && AuroraUtils.dimensionHasAuroras() && CommonState.getTruePlayerBiome().getHasAurora();
    }
    ...
    public void process(@Nonnull final Player player) {
        ...
        if (this.current != null) {
            ...
            // outright
            if (this.current.isComplete() || this.dimensionId != CommonState.getDimensionId() || !Config.CLIENT.aurora.auroraEnabled.get()) {
                this.current = null;
            } else {
                this.current.update();
     ...
     * 
     * @param matrixStack
     *            Matrix stack of the current environment
     * @param partialTick
     *            Partial tick, duh */
