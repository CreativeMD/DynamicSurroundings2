...
public abstract class JetEffect extends BlockEffect {
    ...
    public static final Predicate<BlockState> HOTBLOCK_PREDICATE = (state) -> LAVA_PREDICATE.test(state) || state.getBlock() == Blocks.MAGMA_BLOCK || LIT_FURNACE.test(state);
    
    public JetEffect(final int chance) {
        super(chance);
    }
    
    public static int countVerticalBlocks(@Nonnull final IBlockReader provider, @Nonnull final BlockPos pos, @Nonnull final Predicate<BlockState> predicate, final int step) {
        int count = 0;
    ...
    public static int countCubeBlocks(@Nonnull final IBlockReader provider, @Nonnull final BlockPos pos, @Nonnull final Predicate<BlockState> predicate, final boolean fastFirst) {
        int blockCount = 0;
        for (int k = -1; k <= 1; k++)
            for (int j = -1; j <= 1; j++)
                for (int i = -1; i <= 1; i++) {
