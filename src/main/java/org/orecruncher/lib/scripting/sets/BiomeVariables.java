...
public class BiomeVariables extends VariableSet<IBiomeVariables> implements IBiomeVariables {
    ...
    private Biome biome;
    private final LazyVariable<Set<BiomeDictionary.Type>> biomeTraits = new LazyVariable<>(() -> BiomeDictionary
            .getTypes(RegistryKey.getOrCreateKey(Registry.BIOME_KEY, this.biome.getRegistryName())));
    private final LazyVariable<Set<String>> biomeTraitNames = new LazyVariable<>(() -> this.biomeTraits.get().stream().map(BiomeDictionary.Type::getName)
