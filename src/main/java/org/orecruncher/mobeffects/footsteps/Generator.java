...
public class Generator {
    ...
    protected void produceStep(@Nonnull final LivingEntity entity, @Nullable AcousticEvent event, final double verticalOffsetAsMinus) {
    ...
    protected void simulateJumpingLanding(@Nonnull final LivingEntity entity) {
        ...
        if (this.isFlying && this.isJumping) {
            ...
            if (this.fallDistance > this.VAR.LAND_HARD_DISTANCE_MIN) {
                ...
                playSinglefoot(entity, 0d, speedDisambiguator(entity, Constants.CLIMB, Constants.CLIMB_RUN), this.isRightFoot);
                this.isRightFoot = !this.isRightFoot;
            }
        }
    }
    
    protected AcousticEvent speedDisambiguator(@Nonnull final LivingEntity entity, @Nonnull final AcousticEvent walk, @Nonnull final AcousticEvent run) {
    ...
    protected void simulateBrushes(@Nonnull final LivingEntity entity) {
        ...
        if (current >= this.brushesTime) {
            ...
            if (proceedWithStep(entity) && (entity.getMotion().x != 0d || entity.getMotion().z != 0d)) {
                final int yy = MathStuff.floor(entity.getPosY() - PROBE_DEPTH - entity.getYOffset() - (entity.isOnGround() ? 0d : 0.25d));
    ...
    protected void playSinglefoot(@Nonnull final LivingEntity entity, final double verticalOffsetAsMinus, @Nonnull final AcousticEvent eventType, final boolean foot) {
    ...
    protected void playMultifoot(@Nonnull final LivingEntity entity, final double verticalOffsetAsMinus, final AcousticEvent eventType) {
    ...
    protected boolean shouldProducePrint(@Nonnull final LivingEntity entity) {
        return this.VAR.HAS_FOOTPRINT && Config.CLIENT.footsteps.enableFootprintParticles
                .get() && (this.isOnGround || !(this.isJumping || entity.isAirBorne)) && !entity.isInvisibleToPlayer(GameUtils.getPlayer());
    }
    ...
    @Nullable
    protected Association findAssociation(@Nonnull final LivingEntity entity, final double verticalOffsetAsMinus, final boolean isRightFoot) {
        ...
        final FootStrikeLocation loc = new FootStrikeLocation(entity, xx, minY - PROBE_DEPTH - verticalOffsetAsMinus, zz);
        
        final AcousticResolver resolver = new AcousticResolver(entity.getEntityWorld(), loc, this.VAR.DISTANCE_TO_CENTER);
        ...
        if (result != null && shouldProducePrint(entity)) {
            ...
            if (printPos != null) {
                ...
                final Footprint print = Footprint.produce(style, entity, printPos, rotDegrees, this.VAR.FOOTPRINT_SCALE, isRightFoot);
