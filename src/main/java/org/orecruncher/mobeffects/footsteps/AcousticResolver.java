...
public class AcousticResolver {
    ...
    public AcousticResolver(@Nonnull final IWorldReader world, @Nonnull final FootStrikeLocation loc, final double distanceToCenter) {
        this.world = world;
        this.loc = loc;
    ...
    protected Association findVanillaAssociation() {
        ...
        if (!(state.getMaterial().isLiquid() || state.isAir(this.world, blockpos))) {
            ...
            SoundType soundtype = blockstate.isIn(Blocks.SNOW) ? blockstate.getSoundType(this.world, blockpos, this.loc.getEntity()) : state
                    .getSoundType(this.world, blockpos, this.loc.getEntity());
