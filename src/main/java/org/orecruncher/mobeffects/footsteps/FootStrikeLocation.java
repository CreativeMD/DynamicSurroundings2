...
public final class FootStrikeLocation {
    ...
    protected FootStrikeLocation(@Nonnull final LivingEntity entity, @Nonnull final Vector3d loc, @Nonnull final BlockPos pos) {
        this.entity = entity;
        this.strike = loc;
        this.stepPos = pos;
    }
    ...
    public Vector3d offset(@Nonnull final Direction facing, final float n) {
        return n == 0 ? this.strike : new Vector3d(this.strike.x + facing.getXOffset() * n, this.strike.y + facing.getYOffset() * n, this.strike.z + facing.getZOffset() * n);
    }
